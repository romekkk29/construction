...
import { ProjectInvoice, Project } from "../../backend/types";
import Modal from "../Styles/Modal";

export default function ClienteFacturasComponent() {
  const { user } = useAuth();
  const isAdmin = user?.role_id === 1;
  const canCreate = user?.role_id === 1 || user?.role_id === 2 || user?.role_id === 3;

  const [records, setRecords] = useState<ProjectInvoice[]>([]);
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const [data, projectsData, assignedIds] = await Promise.all([
          apiClient.projectInvoices.list(),
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      await apiClient.projectInvoices.delete(id);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const created = await apiClient.projectInvoices.create(fd);
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filtered.map((r) => (
                  <tr key={r.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 text-sm text-gray-700 whitespace-nowrap">{r.invoiceDate?.toString().split("T")[0]}</td>
                    ...
                    <td className="px-4 py-3 text-center">
                      {r.documentName ? (
                        <a
                          href={`/api/project-invoices/documents/${r.documentName}`}
