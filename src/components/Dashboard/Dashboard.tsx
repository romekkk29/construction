...
import { Construction, Warehouse, ClipboardList, TrendingUp, Filter } from 'lucide-react'; // Agregué Filter
import { 
...
export default function DashBoardComponent() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* BARRA DE FILTROS */}
      <div className="flex justify-end items-center gap-3 bg-white p-4 rounded-2xl border border-slate-100 shadow-sm">
        <label className="text-sm font-semibold text-slate-600 flex items-center gap-2">
          <Filter className="h-4 w-4" /> Filtrar por Obra:
        </label>
        <select 
          value={selectedProject}
          onChange={(e) => setSelectedProject(e.target.value)}
          className="bg-slate-50 border border-slate-200 text-slate-700 text-sm rounded-xl focus:ring-blue-500 focus:border-blue-500 block p-2.5 outline-none transition-all"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
          <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
            ...
            {selectedProject === 'all' ? 'Consumo vs Presupuesto Global' : `Detalle: ${selectedProject}`}
          </h3>
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              {/* USAMOS filteredProjects AQUÍ */}
